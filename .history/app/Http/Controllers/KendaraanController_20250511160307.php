...
class KendaraanController extends Controller
{
    ...
    public function exportPDF()
    {
        ...
        $pdf = PDF::loadView('pdf.kendaraan', compact('kendaraans')); // Membuat view untuk PDF
        return $pdf->download('kendaraan.pdf'); // Mengunduh PDF
    }
    ...
    public function export($format)
    {
        ...
        if ($format == 'excel') {
            return Excel::download(new KendaraanExport($data), 'data_kendaraan.xlsx');
        }

        if ($format == 'pdf') {
            $pdf = PDF::loadView('kendaraan.export_pdf', ['data' => $data]);
